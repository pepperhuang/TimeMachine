...
tolplot <- function(trueHr,predHr,main="Time of Day (24h)",add=FALSE,col=1,...){
...
CPtrain <- all.meta$train==1 & !is.na(all.meta$LocalTime)
...
CPtrainTimes <- all.meta[CPtrain,"LocalTime"]
 
# Train TimeMachine on Ratio
TM.LT.Ratio <- trainTimeStamp(
...
all.meta$TMpredRatio <- predTimeStamp(TM.LT.Ratio,newx=ratio.expr, s = 0.1) 

# Train TimeMachine on ZScore
TM.LT.ZScore <- trainTimeStamp(
...
all.meta$TMpredZScore <- predTimeStamp(TM.LT.ZScore,newx=predictor.expr.final, s = 0.4)

#=====================================================================
# LOAD PLSR prediction from Laing et al.
#=====================================================================
matPredDeg <- read.csv("matlab_PLSR_files/Prediction_PLSR_1sample_5_100_LocalTime.csv",header=F)
colnames(matPredDeg) <- c("LTdeg","matPLpred")
...
# pdf(file="Figure2.pdf",width=11,height=3.3)
par(mfcol=c(1,4))
...
TMauc <- with(all.meta.split$TrTe[all.meta.split$TrTe$train==0,],
              tolplot(LocalTime,TMpredRatio,plot=T,main="Test Set (Moller et al.)",col=cond))
TMZauc <- with(all.meta.split$TrTe[all.meta.split$TrTe$train==0,],
               tolplot(LocalTime, TMpredZScore, add=T,col="purple3",lty=2))
PLSauc <- with(all.meta.split$TrTe[all.meta.split$TrTe$train==0,],
               tolplot(LocalTime, PLmatpred, add=T,col="green4",lty=2))
statLegend()

# V1 validation data (Archer)
TMauc <- with(all.meta.split$V1,
              tolplot(LocalTime,TMpredRatio,plot=T,main="Validation V1 (Archer et al.)",col=cond))
...
TMauc <- with(all.meta.split$V2,
              tolplot(LocalTime,TMpredRatio,plot=T,main="Validation V2 (Arnardottir et al.)",col=cond))
TMZauc <- with(all.meta.split$V2,
               tolplot(LocalTime, TMpredZScore, add=T,col="purple3",lty=2))
...
TMauc <- with(all.meta.split$V3,
              tolplot(LocalTime,TMpredRatio,plot=T,main="Validation V3 (RNA-seq)"))
